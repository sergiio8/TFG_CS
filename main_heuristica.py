...
def graphic_fitness_evolution(fitness_evolution, nombre_problema):
    ...
    plt.plot(generaciones, fitness_values, linewidth=2, color='#ff7f0e', label='Heurística Competente')
    ...
    plt.tight_layout()
    plt.savefig(f"Grafica_Heuristica_{nombre_problema}.png", dpi=300)
    plt.close()

# ==========================================
# 2. GENERADOR DE CONFIGURACIÓN HEURÍSTICA RÍGIDA (ABLACIÓN)
# ==========================================
def generate_competent_heuristic_config(ca_size, num_states, target_state, colors, h_bound, v_bound, use_random_init):
    # =================================================================
    # 1. AUTÓMATA CELULAR (CA) - Entorno idéntico al CBR
    # =================================================================
    ...
    # --- HEURÍSTICA DE DOS TRAMOS ---
    # Tramo 1 (Fitness 0.0 a 0.50): Muta 8 genes (Alta exploración)
    # Tramo 2 (Fitness 0.50 a 1.0): Muta 3 genes (Explotación fina)
    array_intervalos_mut = np.array([0.0, 0.50, 1.0])
    array_num_muts = np.array([8, 3])
...
if __name__ == "__main__":
    ...
    problemas = [
        {
            "nombre": "Bangladesh", "target": target_bangladesh, "estados": 2, "colores": colors_bangladesh,
            "h_bound": "periodic", "v_bound": "periodic", "random_init": False # Semilla Central
        },
        {
